...
{
    public class GptOptionsEntityConfig : IEntityTypeConfiguration<GptOptionsEntity>
    {
        public void Configure(EntityTypeBuilder<GptOptionsEntity> builder)
        {
            //在 PostgreSQL 中，表名以及列名是不区分大小写的。这意味着在数据库中创建表时，无论您是使用大写、小写或混合大小写的名称，最终都会使用相同的名称来访问和操作该表。
            //设置表
            builder.ToTable("GptOptions");
            //设置表主键
            builder.HasKey(e => e.GptOptionsId);
            //设置主键自增
            builder.Property(e => e.GptOptionsId)
                   .UseIdentityColumn();
            builder.Property(e => e.OpenAIOptions).IsRequired(false).HasColumnType("jsonb");
            builder.Property(e => e.ImagOptions).IsRequired(false).HasColumnType("jsonb");
